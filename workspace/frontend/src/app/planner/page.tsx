...
const PLANNER_DAYS = [10, 11, 12];
const PLANNER_EVENTS = [{ day: 11, label: "확정 가능성 높음" }];
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="planner-poll-title" value={title} onChange={(event) => setTitle(event.target.value)} placeholder="예: 주말 가족 나들이" autoFocus />
          </div>
          <div className="space-y-2">
            <Label htmlFor="planner-poll-start">희망 시작일</Label>
            <Input id="planner-poll-start" type="date" min={dateInputValue()} value={startDt} onChange={(event) => setStartDt(event.target.value)} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="planner-poll-end">희망 종료일</Label>
            <Input id="planner-poll-end" type="date" min={startDt} value={endDt} onChange={(event) => setEndDt(event.target.value)} />
          </div>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={!title.trim() || !isValidPeriod || isSubmitting}>{isSubmitting ? "등록 중..." : "등록하기"}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
...
function PlaceSuggestionDialog({
  ...
  const [sourceUrl, setSourceUrl] = useState("https://naver.me/5uIYATBA");
  ...
  const reset = () => {
    setSourceUrl("https://naver.me/5uIYATBA");
    setThumbnailUrl("");
    setTitle("");
    setDescription("");
    setCategory("장소");
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Input id="place-url" type="url" value={sourceUrl} onChange={(event) => setSourceUrl(event.target.value)} />
              <Button type="button" variant="outline" onClick={handlePreview} disabled={!sourceUrl || isResolving}>
                {isResolving ? "가져오는 중..." : "미리보기"}
              </Button>
            </div>
          </div>
          {thumbnailUrl && <div className="h-28 rounded-lg bg-muted bg-cover bg-center" style={{backgroundImage: `url(${thumbnailUrl})`}} />}
          <div className="space-y-2">
            <Label htmlFor="place-title">제목</Label>
            <Input id="place-title" value={title} onChange={(event) => setTitle(event.target.value)} placeholder="장소 이름" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="place-description">설명</Label>
            <Input id="place-description" value={description} onChange={(event) => setDescription(event.target.value)} placeholder="장소 소개" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="place-category">카테고리</Label>
            <Input id="place-category" value={category} onChange={(event) => setCategory(event.target.value)} placeholder="예: 맛집/카페" />
          </div>
          <DialogFooter>
            <DialogClose render={<Button type="button" variant="outline" disabled={isSubmitting}>취소</Button>} />
            <Button type="submit" disabled={!thumbnailUrl || !title.trim() || !description.trim() || !category.trim() || isSubmitting}>
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="expense-item" value={item} onChange={(event) => setItem(event.target.value)} placeholder="예: 숙소 예약" autoFocus />
          </div>
          <div className="space-y-2">
            <Label htmlFor="expense-payer">결제자</Label>
            <Input id="expense-payer" value={payer} onChange={(event) => setPayer(event.target.value)} placeholder="예: 민지" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="expense-amount">금액</Label>
            <Input id="expense-amount" type="number" min="1" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="예: 120000" />
          </div>
          <div className="space-y-2">
            ...
            >
              {Object.entries(EXPENSE_STATUS).map(([value, {label}]) => <option key={value} value={value}>{label}</option>)}
            </select>
          </div>
          <DialogFooter>
            <DialogClose render={<Button type="button" variant="outline" disabled={isSubmitting}>취소</Button>} />
...
export default function PlannerPage() {
  ...
  const plannerWorkspaceId = workspaceId ?? "mock-workspace";
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="mx-auto max-w-6xl space-y-6 px-4 py-5 lg:px-8 lg:py-8">
          <header className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div>
              <h2 className="text-2xl font-bold tracking-tight text-primary lg:text-[32px]">제주도 우정 여행 플래너</h2>
              <p className="mt-2 text-sm text-muted-foreground lg:text-base">함께 가는 사람들: 이주, 민지, 준호 + 게스트 3명</p>
            </div>
            <AvatarGroup>
              <Avatar size="lg"><AvatarFallback className="bg-[#d5e3fc] font-bold text-primary">이</AvatarFallback></Avatar>
              <Avatar size="lg"><AvatarFallback className="bg-[#d4e4fa] font-bold text-primary">민</AvatarFallback></Avatar>
              <Avatar size="lg"><AvatarFallback className="bg-[#dae3f0] font-bold text-primary">준</AvatarFallback></Avatar>
              <AvatarGroupCount>+3</AvatarGroupCount>
            </AvatarGroup>
          </header>

          <div className="grid grid-cols-1 gap-5 lg:grid-cols-12">
            <Card className="gap-0 border-white/70 bg-card/70 py-0 shadow-sm backdrop-blur lg:col-span-8">
              <CardHeader className="p-5 lg:p-6">
                ...
                <div className="flex items-center gap-1">
                  <Button variant="ghost" size="icon" aria-label="이전 달" onClick={() => shiftMonth(-1)}>
                    <ChevronLeft size={18} />
                  </Button>
                  <Button variant="ghost" size="icon" aria-label="다음 달" onClick={() => shiftMonth(1)}>
                    <ChevronRight size={18} />
                  </Button>
                  <Button size="sm" className="ml-1" onClick={() => setPollOpen(true)} disabled={!workspaceId}>
              ...
              <CardContent className="space-y-5 pb-5 lg:pb-6">
                ...
                <div className="space-y-2">
                  ...
                  {isPollLoading && <p className="py-2 text-center text-xs text-muted-foreground">불러오는 중...</p>}
                  {!isPollLoading && schedulePolls.length === 0 && <p className="py-2 text-center text-xs text-muted-foreground">등록된 희망 일정이 없습니다.</p>}
                  {schedulePolls.flatMap((poll) => poll.candidates.map((candidate) => (
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span className={cn("text-xs font-semibold", candidate.votedByMe ? "text-primary" : "text-muted-foreground")}>
                ...
                <div className="flex items-center gap-3 rounded-lg bg-muted/70 p-3 text-sm text-muted-foreground">
            ...
            <Card className="gap-0 border-0 bg-primary py-0 text-primary-foreground shadow-lg lg:col-span-4">
              ...
              <div className="space-y-4">
                ...
                {expenses.slice(0, 3).map((expense) => (
                  <div key={expense.id} className="flex items-center justify-between gap-3 text-sm">
                    ...
                    <span className="font-medium text-[#d5e3fc]">{formatCurrency(expense.amount)}</span>
                  </div>
                ))}
              </div>
              <Button className="mt-6 w-full bg-card text-primary hover:bg-card/90" onClick={() => setExpenseOpen(true)}>내역 추가</Button>
              </CardContent>
            </Card>
            ...
            <section className="space-y-4 lg:col-span-12">
              ...
              <div className="flex flex-col gap-4 sm:flex-row">
                <Button type="button" variant="outline" className="h-64 w-full shrink-0 flex-col border-2 border-dashed text-muted-foreground hover:bg-muted/60 sm:w-52" onClick={() => setPlaceOpen(true)}>
                  <span className="mb-3 flex size-12 items-center justify-center rounded-full bg-muted"><Plus size={22} /></span>
                  <span className="text-sm font-medium">장소 추가하기</span>
                </Button>
                {places.length === 0 && !isPlaceLoading ? (
                  <div className="flex h-64 flex-1 items-center justify-center rounded-xl border border-dashed border-border bg-muted/30 px-6 text-center text-sm text-muted-foreground">
                    장소를 추천해주세요.
                  </div>
                ) : (
                  <Carousel opts={{align: "start"}} className="min-w-0 flex-1 px-8">
                    ...
                    {places.map((place) => (
                    <CarouselItem key={place.id} className="basis-[82%] pl-4 sm:basis-1/2 lg:basis-1/3 xl:basis-1/4">
                      <Card className="h-64 gap-0 py-0 transition-shadow hover:shadow-lg">
                        ...
                        >
                          <Chip color="slate" size="sm" className="absolute left-3 top-3 bg-card/90 backdrop-blur">
                            {place.category}
                          </Chip>
                        </div>
                        <CardContent className="flex flex-1 flex-col justify-between space-y-2 p-4">
                          <div>
                            <a href={place.sourceUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1 font-semibold text-primary hover:underline">
                          ...
                          <div className="flex items-center justify-between gap-2">
                            <span className="text-xs text-muted-foreground">{place.voteCount}명 투표</span>
                            <Button
                              size="sm"
                            ...
                            >
                              <Heart size={14} fill={place.votedByMe ? "currentColor" : "none"} /> 좋아요
                            </Button>
                          </div>
                        </CardContent>
                      </Card>
              ...
              {isPlaceLoading && <p className="text-center text-xs text-muted-foreground">장소를 불러오는 중...</p>}
            </section>

            <Card className="gap-0 border-white/70 bg-card/70 py-0 shadow-sm backdrop-blur lg:col-span-12">
              ...
              <CardContent className="p-5 lg:p-6">
                <Table className="min-w-[600px] text-left">
                  <TableHeader className="text-xs text-muted-foreground">
                    <TableRow>
                      <TableHead>날짜</TableHead><TableHead>항목</TableHead><TableHead>결제자</TableHead><TableHead>금액</TableHead><TableHead className="text-right">상태</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {expenses.map((expense, index) => (
                      <TableRow key={expense.id}>
                        <TableCell className="py-4 text-muted-foreground">{expense.date.replaceAll("-", ".")}</TableCell>
                        <TableCell className="py-4 font-semibold">{expense.item}</TableCell>
                        <TableCell className="py-4"><span className="flex items-center gap-2"><Avatar size="sm"><AvatarFallback className={["bg-[#d5e3fc]", "bg-[#d4e4fa]", "bg-[#dae3f0]"][index % 3]} /></Avatar>{expense.payer}</span></TableCell>
                        <TableCell className="py-4">{formatCurrency(expense.amount)}</TableCell>
                        <TableCell className="py-4 text-right"><Chip color={EXPENSE_STATUS[expense.status].chip} size="sm">{EXPENSE_STATUS[expense.status].label}</Chip></TableCell>
                      </TableRow>
                    ))}
                    {!isExpenseLoading && expenses.length === 0 && (
                      <TableRow><TableCell colSpan={5} className="py-8 text-center text-muted-foreground">등록된 정산 내역이 없습니다.</TableCell></TableRow>
                    )}
                  </TableBody>
                </Table>
              </CardContent>
